/* =============================================================
   ...
   FIX LOG
   -----------------------------------------------------------
   The settings.* keys were previously pasted as a block of loose
   'key': 'value' pairs AFTER the TRANSLATIONS object literal had
   already closed (an "ADDITIONS" section appended past the final
   `};`). A string literal followed by `:` at the top level of a
   script isn't a legal statement, so that made the whole file a
   SyntaxError — TRANSLATIONS never got assigned, window.MeridianI18n
   was never created, and every data-i18n element on every page
   silently kept whatever was hardcoded in its HTML fallback. Each
   settings.* block below has been moved inside its matching
   language object, and the duplicate common.close / common.cancel
   lines from that block have been dropped since those keys already
   exist in every language object.
   ============================================================= */
...
(function () {
  ...
  /* -----------------------------------------------------------
     Follow-ups still needed in pages/settings.js
     -----------------------------------------------------------
     The strings below are generated dynamically in JS (toasts,
     confirm() dialogs, and empty-state fallbacks re-rendered after
     a fetch) so data-i18n on the HTML alone won't reach them. Each
     needs to be swapped for a MeridianI18n.t('key') call once new
     keys are added for them:

       showToast('Preferences saved.')
         -> MeridianI18n.t('settings.general.save_success')  [NEW key]
       showToast('Appearance updated.')                        [x2]
         -> MeridianI18n.t('settings.appearance.save_success') [NEW key]
       window.confirm('Sign out of every device, including this one?')
         -> MeridianI18n.t('settings.danger.confirm')          [NEW key]
       getStatementFormValues(): 'Choose both a start and end date.'
         and 'The start date must be before the end date.'
         -> settings.statements.error_missing_dates /
            settings.statements.error_date_order      [NEW keys]
       showToast('No transactions in that range to export.')
         -> settings.statements.error_empty            [NEW key]
       showToast('Statement downloaded.')               [x2]
         -> settings.statements.download_success        [NEW key]
       loadLinkedAccounts() fallback: "Couldn't load linked accounts."
         and the re-rendered empty state
         -> reuse settings.linked.empty + new settings.linked.load_error
       showToast('External account added — verification usually
         takes 1–2 business days.')
         -> settings.modal.linked.added_toast            [NEW key]
       showToast('External account removed.')
         -> settings.linked.removed_toast                 [NEW key]
       loadConnectedApps()/loadApiKeys() re-rendered empty states
         -> reuse settings.apps.empty / settings.apps.keys.empty
       showToast('Access revoked.') / showToast('API key revoked.')
         -> settings.apps.revoked_toast / settings.apps.keys.revoked_toast
       showToast('Key copied — store it somewhere safe.') and
         showToast("Couldn't copy — select and copy manually.", 'error')
         -> settings.modal.api_key.copy_success / copy_error
       renderMonthlyStatements() month labels use
         toLocaleDateString('en-US', {...}) — these will keep
         showing English month names in every language until that
         call is swapped for the visitor's active locale code.

     Flagging these together rather than translating them now, so
     there's a single merge point instead of two.
     ----------------------------------------------------------- */
